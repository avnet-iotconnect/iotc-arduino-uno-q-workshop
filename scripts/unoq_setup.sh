...
usage() {
  cat <<EOF
Usage: $0 [--demo-dir PATH] [--skip-apt] [--no-systemd] [--skip-sdk] [--no-rename-certs] [--force-config] [--pip-break-system-packages]
...
if [[ "$NO_SYSTEMD" == "0" && "$RESTART_NEEDED" == "1" ]]; then
  systemctl restart iotc-relay.service
  echo "Restarted relay service due to config/cert changes."
fi
